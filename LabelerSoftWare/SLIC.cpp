...
namespace slic
{
	void CopyMatToMem(Mat& image, unsigned int* ubuff, int width, int height)
	{
		assert(ubuff != nullptr);
		for (int i = 0; i < height; i++)
		{
			for (int j = 0; j < width; j++)
			{
				unsigned int B = image.at<cv::Vec3b>(i, j)[0];
				unsigned int G = image.at<cv::Vec3b>(i, j)[1];
				unsigned int R = image.at<cv::Vec3b>(i, j)[2];
				/*		B = B & 0xFF;
				G = G & 0xFF;
				R = R & 0xFF;*/
				unsigned int *data = ubuff + i*width + j;
				*data = ((R << 16) + (G << 8) + B) & 0xFFFFFFFF;
				int r = (ubuff[j] >> 16) & 0xFF;
				int g = (ubuff[j] >> 8) & 0xFF;
				int b = (ubuff[j]) & 0xFF;
				b = 1;
	...
	void CopyMemToMat(Mat& image, unsigned int* ubuff, int width, int height)
	{
		assert(ubuff != nullptr);
